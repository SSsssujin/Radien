#include"framework.h"
#include"image.h"
#pragma comment(lib,"msimg32.lib")				//알파블렌드를 사용하기 위해
...
HRESULT image::init(const int width, const int height)
{
	...
	HDC hdc = GetDC(m_hWnd);																//DC 가져오기

	//이미지 정보 생성
	_imageInfo = new IMAGE_INFO;
	_imageInfo->loadType = static_cast<BYTE>(IMAGE_LOAD_KIND::LOAD_EMPTY);
	_imageInfo->resID = 0;
	_imageInfo->hMemDC = CreateCompatibleDC(hdc);											//비트맵을 출력하기 위해 메모리 DC 생성
	_imageInfo->hBit = (HBITMAP)CreateCompatibleBitmap(hdc, width, height);					//원본 DC와 호환되는 비트맵 생성
